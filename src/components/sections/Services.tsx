...
export default function Services() {
  return (
    <section id="services" className="relative overflow-hidden bg-sand">
      <div
        className="bg-orb pointer-events-none absolute -right-12 top-12 h-44 w-44 opacity-50 md:h-60 md:w-60"
        aria-hidden="true"
      />
      <Reveal className="mobile-reveal mobile-delay-1">
            ...
            {servicePillars.map((pillar, index) => (
              ...
              >
                <Card className="group space-y-4 transition-transform duration-300 ease-out hover:-translate-y-1 hover:shadow-card">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-mist text-brand transition-colors duration-300 ease-out group-hover:bg-brand group-hover:text-white">
                  ...
                  <p className="text-xs font-semibold uppercase tracking-[0.3em] text-brand transition-colors duration-300 ease-out group-hover:text-ink-2">
                    {pillar.tag}
                  </p>
                </Card>
              </Reveal>
